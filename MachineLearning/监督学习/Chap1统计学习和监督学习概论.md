...
## 监督学习应用
- 主要三个方面：分类问题、标注问题、回归问题。

### 分类问题
- 定义 <br /> 输入**任意**、输出**离散**。
- 习得的模型称为**分类器**；分类器对新的输入进行预测称为**分类**；可能的输出称为**类别**。
- 分类问题包括**学习**和**分类**两个过程。
  - 用已知训练集进行学习；
  - 用新输入进行分类。
- [ ] 图1.10
- 评价指标
  - 分类准确率 <br /> 对于给定的测试集，正确分类的样本数与总样本数之比，即$\frac{1}{N^{'}}\sum \limits_{i=1}^{N^{'}}I(y_i = \hat{f}(x_i))$.(联想损失函数为0-1损失时的测试误差)
  - 精确率和召回率
    - 正类 --- 关注的类 <br /> 负类 --- 正类以外的类
    - 分类器预测结果的4种情况的数量
      - TP -- 将正类预测为正类的数目
      - FN -- 将正类预测为负类的数目
      - FP -- 将负类预测为正类的数目
      - TN -- 将负类预测为负类的数目
    - 精确率 <br /> <center> $P=\frac{TP}{TP+FP}$ </center> <br /> 即模型预测出的正类有多少是真正的正类。
    - 召回率 <br /> <center> $R=\frac{TP}{TP+FN}$ </center> <br /> 即对于所有真正的正类，模型预测对了多少个。
    - $F_1$值 <br /> <center> $\frac{2}{F_1}=\frac{1}{P}+\frac{1}{R}$ </center>
    - 这三个指标都是$0$到$1$之间的数，且精确率、召回率都高时，$F_1$也会高。
- 典型方法：k近邻法、感知机、朴素贝叶斯法、支持向量机等。

### 标注问题
- 目标 <br /> 学习一个模型，使其能够对观测序列给出标记序列作为预测。 <br /> 标记个数有限，但*标记个数的组合所形成的标记序列的个数是依序列长度呈指数级增长的*。
- 标注问题分为**学习**和**标注**两个过程。
  - 学习 <br /> 给定训练数据集 <br /> <center> $T=\{(x_1,y_1),(x_2,y_2),...,(x_N,y_N)\}$ </center> <br /> 其中$\bold{x_i}=(x_i^{(1)},x_i^{(2)},...,x_i^{(n)}),i=1,2,...,N$为观测序列；$\bold{y_i}=(y_i^{(1)},y_i^{(2)},...,y_i^{(n)})$为相应的输出标记序列；$n$是序列的长度，对不同样本可以有不同的值，一般$n \ll N$。 <br /> 系统基于训练集构建一个模型，表示为条件概率分布<br /> <center> $P\{Y^{(1)},Y^{(2)},...,Y^{(n)}|X^{(1)},X^{(2)},...,X^{(n)}\}$ </center> <br /> 每一个$X^{(i)}$取值为所有可能的观测，每一个$Y^{(i)}$取值为所有可能的标记。
  - 标注 <br /> 标注系统按照模型，对于新的输入观测序列输出标记序列，即找到 <br /> <center> $\bold{y_{N+1}}=\mathop{argmax}P\{\bold{y_{N+1}}|\bold{x_{N+1}}\})$ </center>
- 常用指标
  - 和分类一样，都是准确率、召回率、精确率，且定义相同。
- 典型方法：隐马尔可夫模型、条件随机场。

### 回归问题
- 目的 <br /> 用于预测输入变量和输出变量之间的关系，特别是二者的微分关系。
- 回归模型等价于函数的拟合，是一种从输入到输出的映射：选择一条曲线使其很好地拟合已知数据并很好地预测未知数据。
- 回归问题分为学习和预测两个过程。
  - 学习 <br /> 给定训练集 <br /> <center> $T=\{(x_1,y_1),(x_2,y_2),...,(x_N,y_N)\}$ </center> <br /> 学习系统根据$T$构建模型$Y=f(X)$;
  - 预测 <br /> 根据习得的模型$Y=f(X)$确定相应的输出$y_{N+1}$.
  - [ ] 图1.12
- 回归问题的分类
  - 按输入变量的个数：一元回归、多元回归。
  - 按输入、输出的关系：线性回归、非线性回归。 